...
func Panel(authenticated bool, user models.Session) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		if authenticated {
			...
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 5, "</div></div><a href=\"/auth/logout\" class=\"btn btn-sm btn-circle flex-shrink-0\" title=\"Log out\"><i class=\"ri-logout-box-r-line text-lg\"></i></a></div><div class=\"divider\"></div><div x-data x-show=\"$store.savedPin.hasPin\" class=\"pin-info\"><legend class=\"fieldset-legend\">My Pin</legend><div class=\"mt-2 text-sm\" x-text=\"$store.savedPin.data.Description\"></div><div class=\"mt-3 flex items-center justify-between\"><button @click=\"$store.savedPin.flyToUserPin()\" class=\"text-blue hover:text-accent text-sm\"><i class=\"ri-map-pin-line\"></i> <span x-text=\"$store.savedPin.data.Latitude.toFixed(4)\"></span>,  <span x-text=\"$store.savedPin.data.Longitude.toFixed(4)\"></span></button> <button @click=\"$store.savedPin.remove()\" class=\"text-red-600 hover:text-accent text-sm\"><i class=\"ri-delete-bin-line\"></i> Remove</button></div><div class=\"divider\"><a class=\"mt-2 text-xs hover:text-accent underline\" x-bind:href=\"'https://atproto-browser.vercel.app/' + $store.savedPin.data.Uri.replace('://', '/')\" target=\"_blank\">View pin on Atproto Browser</a></div></div><div x-data x-show=\"!$store.savedPin.hasPin\" class=\"pin-info\"><p class=\"mt-2 text-sm\">You don't have a pin set. Click the map to place one!</p></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
		} else {
