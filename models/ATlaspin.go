...
// Lexicon schema: io.whiteley.luke.ATlas.pin
...
func init() {
	lexutil.RegisterType("io.whiteley.luke.ATlas.pin", &ATlasPin{})
}

type ATlasPin struct {
	LexiconTypeID string  `json:"$type" cborgen:"$type,const=io.whiteley.luke.ATlas.pin"`
...
func (t *ATlasPin) MarshalCBOR(w io.Writer) error {
	if t == nil {
	...
	if err := cw.WriteMajorTypeHeader(cbg.MajTextString, uint64(len("io.whiteley.luke.ATlas.pin"))); err != nil {
		return err
	}
	if _, err := cw.WriteString(string("io.whiteley.luke.ATlas.pin")); err != nil {
		return err
	}
...
func (t *ATlasPin) UnmarshalCBOR(r io.Reader) (err error) {
	*t = ATlasPin{}
	...
	if extra > cbg.MaxLength {
		return fmt.Errorf("LukeAtlasPin: map struct too large (%d)", extra)
	}
